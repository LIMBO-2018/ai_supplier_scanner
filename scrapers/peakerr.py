...
class PeakerrScraper(BaseScraper):
    name = "Peakerr"
    search_url_template = "https://peakerr.com/search?q={query}"
    trust_base = 60
    ...
    def _parse(self, html: str, query: str, search_url: str) -> List[ProductListing]:
        ...
        cards = (
            soup.select(".service-card")
            or soup.select(".service-item")
            or soup.select(".product-box")
            or soup.select("[class*='service']")
        ...
        for card in cards[:MAX_RESULTS_PER_MARKET]:
            try:
                ...
                logger.debug("Peakerr card parse error: %s", e)
